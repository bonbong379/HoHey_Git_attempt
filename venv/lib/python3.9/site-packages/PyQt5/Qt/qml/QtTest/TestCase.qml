...
/*!
 ...
 height: 50
    ...
    TestCase {
        ...
        function test_click() {
            var item = Qt.createQmlObject("import QtQuick 2.0;
 Item {}", testCase);
    ...
    result in interference with future tests;
 for example, by blocking input
    ...
    TestCase {
        ...
        function test_click() {
            var item = createTemporaryQmlObject("import QtQuick 2.0;
 Item {}", testCase);
...
Item {
    ...
    */
    function isPolishScheduled(item) {
        if (!item || typeof item !== "object") {
            qtest_results.fail("Argument must be a valid Item;
 actual type is " + typeof item,
    ...
    */
    function waitForItemPolished(item, timeout) {
        if (!item || typeof item !== "object") {
            qtest_results.fail("First argument must be a valid Item;
 actual type is " + typeof item,
                util.callerFile(), util.callerLine())
            throw new Error("QtQuickTest::fail")
        }
        ...
        if (timeout !== undefined && typeof(timeout) != "number") {
            qtest_results.fail("Second argument must be a number;
 actual type is " + typeof timeout,
                util.callerFile(), util.callerLine())
            throw new Error("QtQuickTest::fail")
        }
    ...
    */
    function createTemporaryQmlObject(qml, parent, filePath) {
        if (typeof qml !== "string") {
            qtest_results.fail("First argument must be a string of QML;
 actual type is " + typeof qml,
                util.callerFile(), util.callerLine());
        ...
        if (!parent || typeof parent !== "object") {
            qtest_results.fail("Second argument must be a valid parent object;
 actual type is " + typeof parent,
                util.callerFile(), util.callerLine());
        ...
        if (filePath !== undefined && typeof filePath !== "string") {
            qtest_results.fail("Third argument must be a file path string;
 actual type is " + typeof filePath,
                util.callerFile(), util.callerLine());
    ...
    */
    function createTemporaryObject(component, parent, properties) {
        if (typeof component !== "object") {
            qtest_results.fail("First argument must be a Component;
 actual type is " + typeof component,
                util.callerFile(), util.callerLine());
        ...
        if (properties && typeof properties !== "object") {
            qtest_results.fail("Third argument must be an object;
 actual type is " + typeof properties,
                util.callerFile(), util.callerLine());
    ...
    */
    function qtest_destroyTemporaryObjects() {
        for (var i = 0;
 i < qtest_temporaryObjects.length;
 ++i) {
 ...
 // => false in IE and Opera,
  ...
  // @todo improve detection of declarative items
 ...
 ++i) {
    ...
    */
    ...
    /*!
        ...
        interpreted as such;
 instead, the pattern will be passed to
